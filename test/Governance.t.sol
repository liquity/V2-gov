...
contract GovernanceTest is Test {
    ...
    function test_check_correct_vote_calculation() public {
        ...
        // stake LQTY
        lqty.approve(address(userProxy), 50e18);
        governanceInternal.depositLQTY(50e18);
        ...
        (, uint32 countedVoteLQTYAverageTimestamp) = governanceInternal.globalState(); // get the average ts before the allocation

        address[] memory initiatives2 = new address[](1);
        initiatives2[0] = baseInitiative1;
        int176[] memory deltaLQTYVotes2 = new int176[](1);
        deltaLQTYVotes2[0] = 30e18;
        ...
        uint240 votingPowerFromInitiative = governanceInternal.lqtyToVotes(voteLQTY2, block.timestamp, averageStakingTimestampVoteLQTY2);
        
        // 4. compare with voting power returned from lqtyToVotes using correct average timestamp
        uint32 averageTimestamp = governanceInternal.calculateAverageTimestamp(countedVoteLQTYAverageTimestamp, averageStakingTimestampVoteLQTY1, voteLQTY1, voteLQTY2);
        uint240 votingPowerFromLqtyToVotes = governanceInternal.lqtyToVotes(voteLQTY2, block.timestamp, averageTimestamp);
        assertEq(votingPowerFromInitiative, votingPowerFromLqtyToVotes);
    }

    /// forge-config: default.fuzz.runs = 50000
    function test_fuzz_check_correct_vote_calculation(int176 deltaVotes1, int176 deltaVotes2) public {
        // objective: verify that the voting power from an allocation is the same as the lqtyToVotes calculated using an average timestamp
        // 1. allocate and reach the specific path prevInitiativeState.counted == 1 by allocating a sufficient amount to reach the threshold
        // votingThreshold = 5e17
        vm.startPrank(user);
        UserProxy userProxy = UserProxy(payable(governanceInternal.deployUserProxy()));

        // stake LQTY
        lqty.approve(address(userProxy), 50e18);
        governanceInternal.depositLQTY(50e18);

        uint96 userStake = userProxy.staked();  // use the staked amount to clamp vote delta
        // warpTime = warpTime % 63072000; //clamp warpTime by a max of two years
        deltaVotes1 = int176(bound(deltaVotes1, 1, int176(uint176(userStake))));
        deltaVotes2 = int176(bound(deltaVotes2, 1, int176(uint176(userStake))));
        ...
        // 2. allocate again to reach the needed path
        (, uint32 countedVoteLQTYAverageTimestamp) = governanceInternal.globalState(); 

        address[] memory initiatives2 = new address[](1);
        initiatives2[0] = baseInitiative1;
        int176[] memory deltaLQTYVotes2 = new int176[](1);
        deltaLQTYVotes2[0] = deltaVotes2;
        int176[] memory deltaLQTYVetos2 = new int176[](1);
        ...
        // 4. compare with voting power returned from lqtyToVotes using correct average timestamp
        // need to pass in countedVoteLQTYAverageTimestamp from snapshot here
        uint32 averageTimestamp = governanceInternal.calculateAverageTimestamp(countedVoteLQTYAverageTimestamp, averageStakingTimestampVoteLQTY1, voteLQTY1, voteLQTY2);
        uint240 votingPowerFromLqtyToVotes = governanceInternal.lqtyToVotes(voteLQTY2, block.timestamp, averageTimestamp);
        assertEq(votingPowerFromInitiative, votingPowerFromLqtyToVotes);
    }
